...
import '../services/mock_groups.dart';
...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final groups = groupsForEvent(event.id);
      ...
      body: ListView(
        ...
        children: [
          ...
          if (groups.isEmpty)
            ...
            ...groups.map((group) => _GroupTile(group: group, onJoin: () => widget.onJoinGroup(group))),

          const SizedBox(height: 20),

          if (eligible && (groups.isEmpty || allGroupsFull))
            OutlinedButton.icon(
              onPressed: () => widget.onCreateGroup(event),
...
class _GroupTile extends StatelessWidget {
  ...
  const _GroupTile({required this.group, required this.onJoin});
